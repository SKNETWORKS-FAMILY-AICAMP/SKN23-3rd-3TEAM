...
# 🧴 온유(On_You) : AI 피부 분석 · 성분 OCR · 올리브영 제품 추천 챗봇
...
| <img src="assets/왼쪽.png" width="120"> | <img src="assets/왼쪽 얼굴.png" width="120">| <img src="assets/중앙.png" width="120"> | <img src="assets/오른쪽 얼굴.png" width="120"> | <img src="assets/오른쪽 날개.png" width="120"> |
|:---:|:---:|:---:|:---:|:---:|
| **송민채** | **정석원** | **강승원** (팀장)  | **정유선** | **이승연** |

</div>

| 이름 | 담당 업무 |
|:---:|:---|
| 강승원(PM/LLM/BACK) | ResNet50 기반 피부 정량 분석 모델 학습(Fast/Deep), 전성분 추출 모델 학습, LangGraph 파이프라인 설계 및 프롬프트 엔지니어링 |
| 정석원(LLM/BACK) | LLM 응답 생성기 구현 및 LLM 라우터, 올리브영 제품 검증 파이프라인, RAG 기반 답변 보강, FastAPI 백엔드 API 구현 |
| 정유선(FRONT/BACK) | React 프론트엔드 개발, ChromaDB 벡터 DB 구축 및 관리, RAG 파이프라인 구현, 임베딩 모델(ko-sroberta) 연동, FastAPI 연동 |
| 송민채(BACK/DB) | MySQL 데이터베이스 설계(ERD), 사용자/분석/위시리스트 테이블 구조, FastAPI 백엔드 API 설계 및 구현, 분석 이력 저장 로직 |
| 이승연(AWS/UI) | AWS EC2 서버 배포 및 DB 서버 구축, 서비스 인프라 운영, 로고 및 UI 이미지 디자인 제작 |
...
<div align="center">
  <img src="assets/뉴스.png" width="800" alt="news">
</div>

- 이 프로젝트는 사용자의 **피부 사진(빠른분석/정밀분석)**, **벡터DB 기반의 피부 고민 상담**, **제품 전성분 이미지(OCR)**를 입력으로 받아 **피부 상태 분석 → 성분 기반 주의/추천 포인트 요약 → 올리브영 실상품 검증 기반 제품 추천 → 위시리스트 저장**까지 제공하는 **멀티모달 스킨케어 챗봇**입니다.

#### 해결하고자 하는 문제

- **(1) 피부 상태 파악의 어려움**  

  사용자는 사진이나 거울로 상태를 봐도 "무엇이 문제인지(수분/모공/주름/탄력/색소 등)"를 정량적으로 해석하기 어렵습니다. 피부 고민은 개인차가 커서 단순 키워드 검색만으로는 맞춤 루틴/제품을 고르기 어렵습니다.

- **(2) 전성분(성분표) 해석 장벽**  

  전성분 표기는 길고 복잡해 민감성/트러블 유발 가능 성분을 직접 판별하기 어렵습니다. 제품 성분은 "피부 고민/피부 타입"과 함께 해석되어야 비로소 의미가 있습니다.

- **(3) "구매 가능한 추천"의 필요**  

   추천은 많지만 실제로는 상품 상세 페이지가 없는 링크(기획전/브랜드관)가 섞이거나, 재고/품절/검색 불일치로 구매까지 이어지지 않는 문제가 있습니다. 본 프로젝트는 **올리브영 '실제 상품 상세 URL' 검증**을 통해 "존재하는 제품만" 추천합니다.

### 2.2. 프로젝트 목표

> 사용자 피부 사진/성분표 기반으로 **피부 분석 + 성분 분석 + 올리브영 검증 제품 추천**을 제공하는 실사용 가능한 챗봇 구축
...
※ 수집 제외 항목: 학회 회원 전용 콘텐츠, 라이선스 문제 있는 외부 사이트, 과도한 수집 부담 항목 [1][2][4][5]
...
- **입력**: 정면 이미지 1장
- **모델**: 얼굴 영역(area 0~8)별 ResNet50, Sigmoid 출력 (0~1)
- **출력**: moisture(수분), elasticity(탄력), wrinkle(주름), pore(모공), pigmentation(색소) + 각 항목 1~5등급
- **피부타입 판정**: 규칙 기반 알고리즘으로 5가지(건성/지성/복합성/중성/민감성) 확정
- **속도**: ~0.5초
...
| VRAM | ~15GB *(입력 해상도/visual token budget에 따라 변동)* |
...
- **속도-정확도 튜닝 가능**: 입력 해상도(visual token budget)를 제한해 처리시간/VRAM을 조절 가능 *(품질 ↔ 속도 트레이드오프)*

#### Flash Attention 2 사용 이유 & 주의사항
...
- URL이 확인되지 않은 제품은 **절대 추천하지 않음** *(안전장치/신뢰성 핵심 정책)*
...
<div align="center">
  <img src="assets\EC2_RunPodGPU_.jpg" width="800">
</div>
...
│   ├── public/
│   │   └── favicon.icons               # 파비콘 파일(확장자 확인 권장)
│   └── src/
...
│       │   ├── logo.png
│       │   ├── profile.png
│       │   ├── info_1.png, info_2.png
│       │   ├── bot.svg
│       │   ├── animations/             # WebM 로고 애니메이션
│       │   │   ├── logo_idle_1.webm
│       │   │   ├── logo_loop_1.webm
│       │   │   ├── logo_pop_1.webm
│       │   │   └── logo_text.webm
...
| **ML / DL** | ![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white) ![ResNet50](https://img.shields.io/badge/ResNet50-FF6F00?style=flat&logoColor=white) ![Qwen2.5-VL](https://img.shields.io/badge/Qwen2.5--VL-7B-blueviolet?style=flat&logoColor=white) |
...
### 10.1. LLM 라우터 프롬프트 `.format()` 충돌

- **증상**: 로그인 후 메시지 전송 시 `KeyError('"intent"')` 발생
- **원인**: Python `.format()` 메서드가 프롬프트 내 JSON 예시의 `{intent}`를 변수 치환 대상으로 인식
- **해결**: JSON 예시의 중괄호를 `{{`, `}}`로 이스케이프

### 10.2. 피부타입 분류 편향 (복합성/지성 집중)
...
### 10.4. 올리브영 링크가 기획전/브랜드관으로만 잡히는 문제

- **원인**: 검색 결과가 상품 상세보다 콘텐츠 URL 비중이 높음
- **해결**: 상품 상세 URL 우선 필터 + 결과 0개 시 fallback 검색 (2단계 전략)

### 10.5. OCR 모델 CPU 환경 실행 불가

- **원인**: Qwen2.5-VL 7B 모델을 RunPod GPU에서 학습했으나, 실제 서비스 환경은 CPU
- **해결**: 데모용 미리 추출된 전성분 JSON 매칭 테이블 방식으로 대체 (순서 기반 카운터)

### 10.6. LLM 응답에 JSON 구조 노출
...
- **강승원** :
- **정석원** :
- **정유선** :이번 프로젝트에서 ReactJS와 Typescript 기반 프론트엔드와 FastAPI 구축을 처음 경험하며 많은 것을 배웠습니다. 귀여운 이미지 디자인과 디자이너의 도움 덕분에 프론트 개발이 더욱 즐거웠습니다.
